...
//Register for device accelerometer measurements
...
void setup() {
  ...
  Serial.begin(115200);
  while(mpu_begin(MPU_RANGE_2G, MPU_SCALE_2000, MPU_CLCK_PLL_XGYRO) != true)
  {
    Serial.println("Check the wiring!");
    Serial.println(fastRegister8(WHO_AM_I_MPU), HEX);
    delay(1000);
  }
...
bool mpu_begin(MPU_RANGE range, MPU_SCALE scale, MPU_CLCK_SOURCE clck)
{
  ...
  set_clock(clck);
  set_range(MPU_RANGE_2G);
  set_scale(MPU_SCALE_2000);
  return true;
...
{
  char output[15];
  uint8_t x_h  = readRegister8(ACCEL_XOUT_H);
  uint8_t x_l = readRegister8(ACCEL_XOUT_L);
  uint8_t y_h = readRegister8(ACCEL_YOUT_H);
  uint8_t y_l = readRegister8(ACCEL_YOUT_L);
  uint8_t z_h = readRegister8(ACCEL_ZOUT_H);
  uint16_t z_l = readRegister8(ACCEL_ZOUT_L);

  float x_axis = (x_h << 8 | x_l) * .000061f *9.80665f ;
  float y_axis = (y_h << 8 | y_l) * .000061f *9.80665f ;
  float z_axis = (z_h << 8 | z_l) * .000061f *9.80665f ;
  Serial.print("x-axis: ");Serial.print(x_axis);
  Serial.print(" y-axis: ");Serial.print(y_axis);
  Serial.print(" z-axis: ");Serial.print(z_axis);
  Serial.println();
